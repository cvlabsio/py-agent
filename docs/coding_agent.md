...
OpenAI-compatible local providers can run without API keys by default:

- `ollama` defaults to `http://localhost:11434/v1`
- `vllm` defaults to `http://localhost:8000/v1`
- `openai-compatible` defaults to `http://localhost:4000/v1`
- `lm-studio` defaults to `http://localhost:1234/v1`
- `localai` defaults to `http://localhost:8080/v1`

Optional keys for those providers are `OLLAMA_API_KEY`, `VLLM_API_KEY`,
`OPENAI_COMPATIBLE_API_KEY`, `LM_STUDIO_API_KEY`, and `LOCALAI_API_KEY`.
