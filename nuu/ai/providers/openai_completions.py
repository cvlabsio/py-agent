...
_KEY_OPTIONAL_OPENAI_COMPATIBLE_PROVIDERS = {
    "ollama",
    "vllm",
    "openai-compatible",
    "lm-studio",
...
def stream_simple_openai_completions(
    ...
    api_key = normalize_api_key(
        (options.api_key if options else None) or get_env_api_key(model.provider)
    )
    if not api_key and not provider_supports_missing_api_key(model.provider):
